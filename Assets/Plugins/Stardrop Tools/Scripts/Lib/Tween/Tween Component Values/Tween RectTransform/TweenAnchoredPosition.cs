...
{
    public class TweenAnchoredPosition : TweenComponentValues<RectTransform, Vector2>
    {
        public TweenAnchoredPosition(RectTransform targetComponent, Vector2 endValue) : base(targetComponent, endValue)
        {
            startValue = targetComponent.anchoredPosition;
        }

        public TweenAnchoredPosition(RectTransform targetComponent, Vector2 startValue, Vector2 endValue)
            : base(targetComponent, startValue, endValue)
        {
        }

        public TweenAnchoredPosition(int id, RectTransform targetComponent, Vector2 startValue, Vector2 endValue)
            : base(id, targetComponent, startValue, endValue)
        {
        }

        public TweenAnchoredPosition(int id, RectTransform targetComponent, Vector2 startValue, Vector2 endValue, float duration, float delay = 0, EaseType easeType = EaseType.EaseInOutSine, LoopType loopType = LoopType.None, int loopCount = 0, AnimationCurve animationCurve = null, System.Action onCompleteCallback = null, System.Action<Vector2> onValueChangedCallback = null)
