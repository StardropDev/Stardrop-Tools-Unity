...
{
    public class TweenEulerAngles : TweenComponentValues<Transform, Vector3>
    {
        public TweenEulerAngles(Transform targetComponent, Vector3 endValue) : base(targetComponent, endValue)
        {
            startValue = targetComponent.eulerAngles;
        }

        public TweenEulerAngles(Transform targetComponent, Vector3 startValue, Vector3 endValue)
            : base(targetComponent, startValue, endValue)
        {
        }

        public TweenEulerAngles(int id, Transform targetComponent, Vector3 startValue, Vector3 endValue)
            : base(id, targetComponent, startValue, endValue)
        {
        }

        public TweenEulerAngles(int id, Transform targetComponent, Vector3 startValue, Vector3 endValue, float duration, float delay = 0, EaseType easeType = EaseType.EaseInOutSine, LoopType loopType = LoopType.None, int loopCount = 0, AnimationCurve animationCurve = null, System.Action onCompleteCallback = null, System.Action<Vector3> onValueChangedCallback = null)
