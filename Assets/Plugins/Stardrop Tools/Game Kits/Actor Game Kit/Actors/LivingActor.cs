...
{
    public abstract class LivingActor : BaseActor, IDamageable, IHealable
    {
        [SerializeField] protected ActorHealth health;
        ...
        public void InitializeHealthComponent(int health = 100, int maxHealth = 100)
        {
            if (IsInitialized)
            {
                Debug.LogWarning("Entity is already initialized.");
                return;
            }

            if (this.health == null)
            {
                this.health = gameObject.AddComponent<ActorHealth>();
